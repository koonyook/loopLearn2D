...
#============parameters=============
folder='circleDenoiseBFGS_25_0.04/'		#this folder will be generated automatically
selectedTransform=circleTransform #choose between circleTransform,ellipseTransform,beanTransform

#===================================

trainsetNormalFile=folder+"trainsetNormal.dat"
...
if genNewData and runTraining:
	patches,not_used=sampleFromFunction(selectedTransform,trainsetSize)
	noisyPatches = addGaussianNoise(patches,0.04)
	patches.dump(trainsetNormalFile)
	noisyPatches.dump(trainsetNoisyFile)
else:
	patches=np.load(trainsetNormalFile).astype(np.float32)
...
def objectiveAndGradient(params):
	W1_,b1_,W2_,b2_=deserialize(params)
	cost_,W1g,b1g,W2g,b2g = sess.run([cost,W1_grad,b1_grad,W2_grad,b2_grad], feed_dict={W1:W1_,b1:b1_,W2:W2_,b2:b2_,x:noisyPatches,expectedOutput:patches})
